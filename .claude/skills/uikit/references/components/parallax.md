# Parallax
> Animate CSS properties depending on the scroll position of the document.  ·  Media · JS
...
## JS — attr `uk-parallax="…"` · api `UIkit.parallax(el, opts)`
...
|---|---|---|
| `easing` | `1` | Animation easing during scrolling |
| `target` | `false` | Element dimension reference for animation duration. |
| `start` | `0` | Start offset. The value can be in vh, % and px. It supports basic mathematics operands + and -. The default value of `0` means that the target's top border and viewport's bottom border intersect. |
| `end` | `0` | End offset. The value can be in vh, % and px. It supports basic mathematics operands + and -. The default value of `0` means that the target's bottom border and the viewport's top border intersect. |
